    ...
    <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border p-6 hover:shadow-2xl transition">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
            {title}
            {verified && <BadgeCheck className="text-green-600" size={20} />}
          </h2>
      ...
      {tags.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {tags.map((tag, idx) => {
            // Color-code tags
            const bgColor = tag.toLowerCase().includes("urgent")
      ...
      <div className="mt-4 space-y-2 text-sm text-gray-700">
        ...
        {postedDate && (
          <div className="flex items-center gap-2 text-gray-600">
            <Calendar size={16} /> <span>Posted: {postedDate}</span>
          </div>
        )}

        {stipend && (
          <div className="flex items-center gap-2 text-gray-600">
            <DollarSign size={16} /> <span>{stipend}</span>
          </div>
        )}

        {estimatedCompensation && (
          <div className="flex items-center gap-2 text-gray-600">
            <BarChart size={16} /> <span>Estimated: {estimatedCompensation}</span>
          </div>
        )}
      </div>
      ...
      <button
        onClick={onApply}
        className="mt-6 w-full bg-orange-500 text-white py-2 rounded-xl hover:bg-orange-600 transition font-medium"
      >
        Apply Now
      </button>
    </div>
